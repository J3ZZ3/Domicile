...
import { FaSwimmingPool, FaParking, FaWind, FaStar, FaStarHalf, FaRegStar } from 'react-icons/fa';
...
const RatingStars = React.memo(({ rating, numberOfRatings }) => {
      ...
      try {
        ...
        if (roomDoc.exists()) {
          ...
          setRoom({
            id: roomDoc.id,
            ...roomData,
            imageUrl: roomData.imageUrl || '/placeholder-room.jpg',
            currentBookings: roomData.currentBookings ?? 0,
            maxBookings: roomData.maxBookings ?? 5,
          });
        } else {
        ...
        if (!cancelled) navigate('/client-dashboard');
      } finally {
  ...
  const handleBookNow = () => {
    if (room.status?.toLowerCase() === 'available') {
      navigate(`/booking/${roomId}`, {
        state: { room, imageUrl: room.imageUrl }
      });
    }
  };
      ...
      <div className="room-detail-content">
        <div className="room-image-hero">
          <div className="room-image-container">
            <img src={room.imageUrl} alt={room.name} className="room-main-image" loading="lazy" />
            <div className="rating-overlay">
